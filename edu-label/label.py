...
#借助第一个函数获得待切割位置和长宽后就可以切割了,切割后放在char文件夹下
...
#用来标注单个字符图片，在label文件夹下，很明显可以看到_后面的就是标签。比如图片里是数字6，_后面就是6。自己手动标注的，其实也可以自己一点点搞，你用程序也行。
def label_data():
...
if __name__ == "__main__":
    #分割图片，调用此程序需要将dels.py文件和train文件夹放在一个目录下。
    label_data()
