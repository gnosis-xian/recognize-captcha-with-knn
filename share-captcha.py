...
def request_share_captcha_base64():
    # result = requests.get("http://yjtgp-web.yto.net.cn/api/loginController/getCaptchCode")
    result = requests.get("http://share.test.manager.web.ng.agno.top/api/loginController/getCaptchCode")
    data = json.loads(result.content)["data"]
    return data["img"], data["captchaToken"]

def save_base64_cap_to_local(bs64):
    img_data = base64.b64decode(bs64)
    img_path = "./share-captcha/cap.jpg"
    with open(img_path, 'wb') as f:
        f.write(img_data)
    return img_path

def get_captcha_result_from_redis(token):
    r = redis.StrictRedis(host='192.168.207.52', port=6380, db=12)
    return str(r.get("manager:login:captch:" + token), encoding = "utf-8")
...
if __name__ == "__main__":
    ...
    right_count = 0
    while True:
        captcha_base64, token = request_share_captcha_base64()
        true_captcha_result = get_captcha_result_from_redis(token)
        filepath = save_base64_cap_to_local(captcha_base64)
        im = cv2.imread(filepath)
        preds = get_code(im)
        calced_ret = preds[0]+preds[1]+preds[2]+preds[3]

        judgement = calced_ret == true_captcha_result
        right_count = right_count+1 if judgement else right_count
        count += 1

        print("【{}】\t识别结果：{}\t真实结果：{}\t准确率：{}".format(judgement, calced_ret, true_captcha_result, right_count / ++count))
